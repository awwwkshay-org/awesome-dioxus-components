...
| Upstream prop-parity evidence | `cargo xtask prop-parity check` for any `registry/ui/*.rs`/`adico-primitives` prop change or `catalog fetch` refresh | Offline, added by `extend-upstream-prop-evidence`; one generated, committed `statics/prop_parity/<item>.json` per registry item, fully derived (no hand-edited fields) so `check` is a plain regenerate-and-compare |
| Playground demo controls | `cargo xtask playground-controls check` for any `apps/web/src/components/ui/*.rs` change; `cargo check --locked --workspace` also fails independently on variant drift via each generated file's compile-time exhaustiveness guard | Offline, added by `generate-playground-controls-from-props`; one generated, committed Rust file per component with an enum-typed prop under `apps/web/src/generated/controls/` |
...
| Responsive layout (viewport) | `cd tests/playwright && npm run test:responsive` (375px, no horizontal overflow) and `test:responsive-desktop` (1280px, desktop geometry unchanged) for any `registry/ui/*.rs` layout/dimension class change | Harness added by `make-registry-components-mobile-first` (2026-09); `tests/playwright/responsive.spec.ts` / `responsive-desktop.spec.ts` run against `apps/web`'s shell-free `/responsive/flow` and `/responsive/overlay?case=` routes. All 69 registry components swept to a mobile-first baseline and verified: 27/27 mobile-viewport assertions and 10/10 desktop-invariance assertions pass. Touch-target sizing (`pointer-coarse:`) is a deliberately separate, not-yet-scheduled follow-up. |
