...
| Responsive layout (viewport) | `cd tests/playwright && npm run test:responsive` (375px, no horizontal overflow) and `test:responsive-desktop` (1280px, desktop geometry unchanged) for any `registry/ui/*.rs` layout/dimension class change | Harness added by `make-registry-components-mobile-first` (2026-09); `tests/playwright/responsive.spec.ts` / `responsive-desktop.spec.ts` run against `apps/playground`'s shell-free `/responsive/flow` and `/responsive/overlay?case=` routes. All 69 registry components swept to a mobile-first baseline and verified: 27/27 mobile-viewport assertions and 10/10 desktop-invariance assertions pass. Touch-target sizing (`pointer-coarse:`) is a deliberately separate, not-yet-scheduled follow-up. |
