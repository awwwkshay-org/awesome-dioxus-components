...
//! this directory, routed under the site's `/playground/*` prefix.
//! `index.rs` is the `/playground` index route, named after TanStack
//! Start's file-based-routing convention for a directory's index route
//! (`routes.rs`'s `Route` enum still declares every path explicitly --
