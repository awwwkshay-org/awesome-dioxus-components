...
/// ```rust
/// use dioxus::prelude::*;
/// use adico_primitives::toast::{ToastOptions, ToastProvider, use_toast};
/// use std::time::Duration;
///
/// #[component]
/// fn Demo() -> Element {
...
pub fn Toast(props: ToastProps) -> Element {
    ...
    if !props.permanent && props.duration.is_some() {
        ...
        // Start a one-shot dismissal timer when the toast mounts. A toast's
        // duration never changes after creation, so a single spawned sleep
        // is equivalent to (and simpler than) a restartable timeout hook.
        use_effect(move || {
            spawn(async move {
    ...
    rsx! {
        div {
            ...
            "data-toast-odd": (!props.index.is_multiple_of(2)).then_some("true"),
