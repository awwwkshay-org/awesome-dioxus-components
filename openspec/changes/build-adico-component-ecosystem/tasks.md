...
- [ ] 4.8a Complete the Tailwind v4 CSS pipeline that M2's `adico init` CSS installer (task 3.11) only half-built: `plan_theme_install` in `packages/adico-cli/src/css.rs` writes the `@theme`/token block but never writes the `@import "tailwindcss";` bootstrap line or a `@source` content-scanning directive, and no installed app links the compiled stylesheet (`document::Stylesheet { href: asset!(...) }`) — found while trying to actually view migrated components styled in a browser, where every installed example rendered as unstyled semantic HTML. Fix the CSS installer to emit a working bootstrap, and either auto-insert the `Stylesheet` link during `adico init`/app-entrypoint wiring or document the one-line requirement clearly; verify a freshly `adico init`'d + `adico add`'d consumer actually renders Tailwind-generated styles under `dx serve`, not just installs token files.
- [ ] 4.8b Build `apps/playground` into an interactive component explorer: for each currently migrated registry item (Wave 1/3/4 plus button/dialog/select), expose it with live-editable props (toggles/selects/text inputs bound to component state) so changes re-render immediately in the browser, installed through the real `adico` CLI (no direct registry workspace imports) and depending on 4.8a for working styles; verify it covers every currently migrated item and stays refreshable as later waves land.
