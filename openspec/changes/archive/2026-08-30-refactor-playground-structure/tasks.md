...
- [x] 1.1 Create `apps/playground/src/routes.rs`; move the `Route` enum,
  ...
  imported).
- [x] 1.2 Update `main.rs` to declare `mod routes;` and use
...
- [x] 2.1 Move `apps/playground/src/demo.rs` and
...
- [x] 2.2 Run `adico add <any already-installed item> --replace` (or
...
- [x] 2.3 Do **not** move `theme.rs` — leave it exactly where and as it is
  (see design.md Decision 5); `main.rs`/`routes.rs`'s `use crate::theme::
  {...}` stays unchanged in this change.

## 3. Split `pages.rs` into `pages/`

- [x] 3.1 Create `apps/playground/src/pages/index.rs` (TanStack Start's
...
- [x] 3.2 Create one file per remaining page under `apps/playground/src/pages/`
...
- [x] 3.3 Move the two `#[cfg(all(test, feature = "server"))]`
...
- [x] 3.4 Create `apps/playground/src/pages/mod.rs` declaring `pub mod`
...
- [x] 3.5 Update `main.rs` and `routes.rs` imports to `mod pages;` /
...
- [x] 4.1 Run `cargo check --workspace`, `cargo fmt --all --check`, and
...
- [x] 4.2 Run a live `dx serve --platform web` instance of
...
- [x] 4.3 Run `openspec validate refactor-playground-structure --strict`;
  verify it reports no errors.

## 5. Record follow-up scope

- [x] 5.1 Confirm no task in this change added a page or route for any of
...
- [x] 5.2 Confirm `theme.rs` was left untouched by this change (see
