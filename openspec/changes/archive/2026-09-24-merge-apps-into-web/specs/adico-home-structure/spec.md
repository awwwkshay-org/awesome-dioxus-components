...
**Reason**: `apps/home` no longer exists as a standalone crate — it is
merged into `apps/web`, whose routing conventions are now governed by
`adico-web-structure`'s "Router definitions live in routes.rs" requirement.
...
**Reason**: Superseded by `adico-web-structure`'s "apps/web is initialized
and kept current through the real adico CLI".
**Migration**: See `adico-web-structure`.
...
**Reason**: Superseded by `adico-web-structure`'s "apps/web has its own
per-project Tailwind pipeline" — one merged app now needs exactly one
pipeline, covering what were previously three.
**Migration**: See `adico-web-structure`.
...
retargeted to `apps/web`'s landing route.
**Migration**: See `adico-web-structure`.
