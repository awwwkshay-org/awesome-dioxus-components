...
**Reason**: `apps/playground` no longer exists as a standalone crate — it is
merged into `apps/web`, whose routing conventions (main.rs holds only the
entrypoint; routes.rs holds every `Route` variant and layout shell) are now
governed by `adico-web-structure`'s "Router definitions live in routes.rs"
requirement.
**Migration**: See `adico-web-structure`.
...
**Reason**: Superseded by `adico-web-structure`'s "One page per file under
pages/, named by route segment", which extends the same convention across
`apps/web`'s landing, docs, and playground page trees.
**Migration**: See `adico-web-structure`.
...
**Reason**: Superseded by `adico-web-structure`'s identical requirement,
retargeted to `apps/web`.
**Migration**: See `adico-web-structure`.
...
**Reason**: Superseded by `adico-web-structure`'s "Playground shell composes
real registry components, not app-specific reimplementations", retargeted
to `apps/web`'s `PlaygroundLayout`.
**Migration**: See `adico-web-structure`.
...
**Reason**: Superseded by `adico-web-structure`'s identical requirement,
retargeted to `apps/web`'s `PlaygroundLayout`.
**Migration**: See `adico-web-structure`.
...
**Reason**: Superseded by `adico-web-structure`'s "The `>= md` playground
layout is unaffected by mobile support", retargeted to `apps/web`'s
`PlaygroundLayout`.
**Migration**: See `adico-web-structure`.
...
**Reason**: Superseded by `adico-web-structure`'s "Registry components are
never modified solely for an app-section's convenience", generalized
across the whole merged app.
**Migration**: See `adico-web-structure`.
...
**Reason**: Superseded by `adico-web-structure`'s identical requirement,
retargeted to `apps/web`'s `/playground/*` routes.
**Migration**: See `adico-web-structure`.
...
**Reason**: Superseded by `adico-web-structure`'s "Shell-free harness routes
stay outside every layout", which strengthens the constraint to also
exclude the site-wide `SiteLayout`, not only the playground's own layout.
**Migration**: See `adico-web-structure`.
